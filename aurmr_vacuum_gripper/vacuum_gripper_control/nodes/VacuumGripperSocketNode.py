...
class VacuumGripper:
    ...
    def getStatus(self):
        if self.gripper_type == RobotiqCModelURCap.GripperType.VACUUM:
            ...
            message.DEVICE_STATUS = self._get_var(self.ACT)
            message.EJECTOR_STATUS = self._get_var(self.MOD)
            message.SUPPLY_VOLTAGE = self._get_var(self.GTO)
            message.LEAKAGE_RATE = self._get_var(self.STA)
            message.FREE_FLOW_VACUUM = self._get_var(self.OBJ)
            message.MAX_VACUUM_REACHED = self._get_var(self.FLT)
            message.SYSTEM_VACUUM = self._get_var(self.PO)
        else:
